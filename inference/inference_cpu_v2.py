...
def main(args):
    ...
    print("param:",(2796816+454776)/1e6)
    # Load  Model
    ...
    for i in range(num_repeat):
        with torch.no_grad():  # 在其内部的所有操作中禁用梯度计算，禁止计算图的构建，从而节省内存和加速运算
            ...
            # create dual quaternions
            sparse_dqs = from_rotation_translation(rotations[:, param["sparse_joints"]],translations[:, param["sparse_joints"]])
            sparse_dqs = dquat.unroll(sparse_dqs, dim=0).to(device)  # ensure continuity (T,6,8)
            sparse_dqs = torch.flatten(sparse_dqs, 1, 2).permute(1, 0)  # (6*8,T)
    ...
    print("time delay = ", time_ms / num_repeat, " ms, fps=",1 / time_ms * num_repeat * 1000)
