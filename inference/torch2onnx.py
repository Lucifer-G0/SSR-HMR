...
from net.skeleton_generator_architecture import Generator_Model, Run_Generator_Model
from net.motion_data import Run_Data
from net.ik_architecture import IK_Model, Run_IK_Model
...
def main(args):
    ...
    run_ik_model = Run_IK_Model(device, param, xsens_parents).to(device)
    # Load  Model
    ...
    # 准备输入张量
    sparse_input_ = torch.randn(1, 48, 64)  # 创建随机输入张量
    res_decoder_ = torch.randn(1, 1, 176)  # 创建随机输入张量
    sparse_dqs_ = torch.randn(1, 1, 48)  # 创建随机输入张量
    offsets_ = torch.randn(1, 1, 66)  # 创建随机输入张量

    # 导出为 ONNX 格式
    output_onnx = "generator_model.onnx"  # 输出文件名
    torch.onnx.export(
        ...
        dynamic_axes={"input": {2: "T"},  # 动态轴设置
                      "output": {1: "T"}}
    )
    print(f"模型已成功导出为 ONNX 格式，并保存到 {output_onnx}")

    # 导出为 ONNX 格式
    output_onnx = "ik_model.onnx"  # 输出文件名
    torch.onnx.export(
        run_ik_model,  # 要导出的模型
