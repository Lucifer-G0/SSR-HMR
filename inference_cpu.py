...
from pymotion.rotations.dual_quat_torch import from_rotation_translation

import time


# 为评估真实环境时间效率定制，帧率数据处理时从120降频为60
def time_test(rotations, translations, offsets, parents, global_pos, run_data, generator_model, ik_model, device='cpu'):
    ...
    offsets = offsets.flatten(start_dim=0, end_dim=1).unsqueeze(-1)

    run_data.set_motions(
        sparse_dqs.unsqueeze(0),
        offsets.unsqueeze(0),
    )
    res_generator = generator_model.forward()
    result = ik_model.forward(res_generator)
    ...
    _, rots = from_root_dual_quat(dqs, parents)
    rots = rots.to(device)
    joint_poses, _ = fk(rots, global_pos, offsets.squeeze(-1).reshape(1, 22, 3), parents)
...
def main(args):
    ...
    # 加载 ONNX 模型
    model_path = "generator_model.onnx"
    ort_session = ort.InferenceSession(model_path)
    ik_model_path = "ik_model.onnx"
    ik_ort_session = ort.InferenceSession(ik_model_path)
    ...
    for i in range(num_repeat):
        with torch.no_grad():  # 在其内部的所有操作中禁用梯度计算，禁止计算图的构建，从而节省内存和加速运算
            ...
            # 准备输入数据（示例输入）
            sparse_dqs = sparse_dqs.unsqueeze(0).numpy()
            # 运行推理
            ...
            outputs_ik = ik_ort_session.run(None, {"input1": outputs_generator[0],
                                                   "input2": np.swapaxes(sparse_dqs[:, :, 42:], 1, 2),
                                                   "input3": offsets.unsqueeze(0).permute(0, 2, 1).numpy()})

            result = torch.tensor(outputs_ik[0])
            # Evaluate Positional Error
            result = result.flatten(0, 1)
            dqs = result.reshape(result.shape[0], 22, 8)
            # get rotations and translations from dual quatenions
    ...
    print("one frame time length = ", 1 / 60 * 1000, " ms time delay = ", time_ms / num_repeat, " ms, fps=",
          1 / time_ms * num_repeat * 1000)
