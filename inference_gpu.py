...
def main(args):
    ...
    for i in range(num_repeat):
        ...
        with torch.no_grad():  # 在其内部的所有操作中禁用梯度计算，禁止计算图的构建，从而节省内存和加速运算
            ...
            # 准备输入数据（示例输入）
            input_data = sparse_dqs.unsqueeze(0).cpu().numpy()
            # 运行推理
            outputs_generator = ort_session.run(None, {input_name: input_data})

            # 运行推理
            outputs_ik = ik_ort_session.run(None, {"input1": outputs_generator[0],
                                                   "input2": sparse_dqs.unsqueeze(0)[:, :, 42:].permute(0, 2,
                                                                                                        1).cpu().numpy(),
                                                   "input3": offsets.unsqueeze(0).permute(0, 2, 1).cpu().numpy()})
